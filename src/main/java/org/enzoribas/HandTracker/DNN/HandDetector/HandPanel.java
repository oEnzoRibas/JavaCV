package org.enzoribas.HandTracker.DNN.HandDetector;
// HandPanel.java
...
public class HandPanel extends JPanel implements Runnable
{
  ...
  */
  {
    ...
    detector = new HandDetector("org.enzoribas/HandTracker/DNN/HandDetector/gloveHSV.txt", WIDTH, HEIGHT);
  ...
  private IplImage picGrab(FrameGrabber grabber, int ID) {
    IplImage im = null;
    try {
      org.bytedeco.javacv.Frame frame = grabber.grab(); // pega o frame da câmera
      if (frame != null) {
        OpenCVFrameConverter.ToIplImage converter = new OpenCVFrameConverter.ToIplImage();
        im = converter.convert(frame); // converte para IplImage
      }
    } catch (Exception e) {
      System.out.println("Problem grabbing image for camera " + ID + ": " + e.getMessage());
    }
    return im;
  }
  ...
  public BufferedImage convertToBufferedImage(IplImage iplImage) {
    OpenCVFrameConverter.ToIplImage converter = new OpenCVFrameConverter.ToIplImage();
    org.bytedeco.javacv.Frame frame = converter.convert(iplImage);
  ...
  */
  { 
    ...
    if (snapIm != null) {
      BufferedImage bufIm = convertToBufferedImage(snapIm);
      g2d.drawImage(bufIm, 0, 0, this);
    }
    if (detector != null)
